...
description: Diagnose native C/C++ runtime bugs with qwen-dap-mcp and DAP. Use for crashes, bad runtime state, unexpected variable changes, breakpoint-driven investigation, and verifying native fixes with CodeLLDB or another authorized local DAP target.
---
...
This skill is for software and local processes the user owns or is authorized to debug.
...
Prefer `debug_start_codelldb` / `debug_launch_codelldb` for Windows native C/C++ projects. Use the generic `debug_start`, `debug_launch`, and `debug_attach` tools only when another DAP adapter is intentionally configured.

## Operating rules

1. Debug only user-owned or otherwise authorized local targets.
...
6. Preserve raw debugger evidence in the diagnosis: stop reason, frame, source line, relevant locals, and the debugger operation that reproduced the issue.
...
At the first relevant stop, call:
...
1. `stopped.reason` and description
2. selected thread
3. top frame and source location
4. stack frames leading to the stop
5. locals / arguments
6. registers when exposed
7. disassembly around the instruction pointer
8. loaded module for the active frame
9. exception information when the stop reason is `exception`

Do not immediately assume the top frame is the root cause. Follow the call stack and data flow back to the first frame controlled by the project when the stop occurs inside a runtime or system library.

## Phase 3: Choose the smallest diagnostic action

### Suspected bad branch or value

Use `debug_evaluate` for a specific expression and `debug_step` to cross the relevant statement. Capture another `debug_snapshot` after the step.

Compare before/after state and identify the exact source operation that changes behavior.

### Variable changes unexpectedly

Use a data breakpoint/watchpoint rather than repeatedly stepping:
...
Use the `instructionPointerReference` from a frame or an address obtained from `debug_disassemble`, then use `debug_set_instruction_breakpoints`.

Instruction breakpoints are a focused fallback when source mapping is unavailable or the issue is instruction-level.
...
Inspect `exceptionBreakpointFilters` from adapter capabilities and configure only the useful filter with `debug_set_exception_breakpoints`.
...
- the exact reproducible action,
- the debugger stop reason,
- the project frame/source line where the problematic state becomes visible,
- the relevant before/after variable or watchpoint evidence,
...
3. Start a fresh debugger session rather than relying on stale module/symbol state.
4. Recreate the same breakpoints/watchpoints or exception filters.
5. Reproduce the original scenario.
6. Confirm the bad stop/value no longer occurs.
7. Capture a final `debug_snapshot` at the corresponding state.
